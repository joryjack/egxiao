(function($, doc) {
	...
	var notelessonstate = [];
	var teacher_name, notelesson_start, notelesson_end, create_name, lesson_name, subject_name;
	...
	function pulldownRefresh() {
		...
		setTimeout(function() {
			...
			app.GetNoteLesson(notelessonRequest, function(data) {
				...
				currentPageIndex = 1;
				var notelessonMainwebview = plus.webview.getWebviewById('singlestu_notelesson_main.html');
	...
	function againNotelesson(notelessondata) {
		...
		app.getSubjectList(account.org_id, function(data) {
			...
			localStorage.setItem("$singlestunotelessondata", notelessondata);
